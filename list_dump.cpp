...
int verify_list(list_t *list) {
    ...
    int previous = 0;
    int count_els = 0;
    if (tail < 0 || head < 1 || size < 0) {
        add_error(ERR_INVALID_ARGUMENTS, "tail: %d, head: %d, last_free: %d, size: %d", tail, head, free, size);
        return 0;
    }
        ...
        if ((errors & 1) == 1) {
            ...
            switch((lstErrors) count) {
                ...
                case ERR_INVALID_NEXT:
                    ...
                    fprintf(fp, "error%d[weight=1,color=\"#ff0000ff\", minlen=4, style=\"filled\", fillcolor=\"#e93131b4\", label=\"invalid %s\"]\n"
...
error_t create_dot_image_dump(list_t *list) {
    ...
    current = list->head;
    count_els = 0;
    while (current <= list->size + 1 && count_els++ <= list->size + 3) {
        int prev = list->prev[current];
        if (prev > -1 && prev <= list->size + 1 && current != prev)
